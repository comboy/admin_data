var AdminData = AdminData || {};

AdminData.actOnResult = {

	action: function(action_type, additional_payload) {
		var formData = $('#advance_search_form').data('admin_data_form_array'),
		parameterizedData;

		formData.push({
			'name': 'admin_data_advance_search_action_type',
			'value': action_type
		});

		if (action_type === 'update_all') {
			formData.push({
				'name': 'advance_search_update_all',
				'value': additional_payload
			});
		}

		parameterizedData = $.param(formData);

		$.ajax({
			url: $('#advance_search_form').attr('action'),
			type: 'post',
			dataType: 'json',
			data: parameterizedData,
			success: function(json) {
				AdminData.actOnResult.successCallback(json);
			}
		});
	},

	successCallback: function(json) {
		$('#results').text(json.success);
	}

};

$('#advance_search_update_all').live('click', function() {
	// remove any existing form
	$('#update_all_umbrella').remove();

	$.get('/admin_data/search/update_all', function(res) {
		$('#search_title_umbrella').append(res);
		$('#update_all_input').focus();
	});

	return false;
});

$('#update_all_submit').live('click', function() {
	AdminData.actOnResult.action('update_all', $('#update_all_input').val());
	return false;
});

$('#advance_search_delete_all').live('click', function() {
	if (window.confirm('Are you sure?')) {
		AdminData.actOnResult.action('delete');
	}
	return false;
});

$('#advance_search_destroy_all').live('click', function() {
	if (AdminData.jsUtil.confirm('Are you sure?')) {
		AdminData.actOnResult.action('destroy');
	}
	return false;
});

